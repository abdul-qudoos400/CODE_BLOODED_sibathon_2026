...
class LoginPage(tk.Frame):
    def __init__(self, parent, app: App):
        ...
        back = tk.Label(phone, text="‹", bg=COLORS["login_card"], fg=COLORS["login_muted"], font=("Segoe UI", 18, "bold"))
        ...
        blob.create_text(60, 110, text="Welcome\nBack", fill=COLORS["login_text"],
                         font=("Segoe UI", 18, "bold"), anchor="w")

        form = tk.Frame(phone, bg=COLORS["login_card"])
        form.place(x=28, y=230, width=274)

        tk.Label(form, text="Username", bg=COLORS["login_card"], fg=COLORS["login_muted"], font=("Segoe UI", 9)).pack(anchor="w")
        self.e_user = tk.Entry(form, bd=0, bg=COLORS["login_card"], fg=COLORS["login_text"], font=("Segoe UI", 11))
        self.e_user.pack(fill="x", pady=(4, 2))
        tk.Frame(form, bg=COLORS["underline"], height=1).pack(fill="x", pady=(0, 18))

        tk.Label(form, text="Password", bg=COLORS["login_card"], fg=COLORS["login_muted"], font=("Segoe UI", 9)).pack(anchor="w")
        self.e_pass = tk.Entry(form, bd=0, show="•", bg=COLORS["login_card"], fg=COLORS["login_text"], font=("Segoe UI", 11))
        ...
        tk.Label(sign_row, text="Sign in", bg=COLORS["login_card"], fg=COLORS["login_text"],
        ...
        link1 = tk.Label(bottom, text="Sign up", bg=COLORS["login_card"], fg=COLORS["login_muted"],
                         font=("Segoe UI", 9, "underline"), cursor="hand2")
        link1.pack(side="left")
        link1.bind("<Button-1>", lambda e: self.app.show_frame("SignupPage"))

        link2 = tk.Label(bottom, text="Forgot Password", bg=COLORS["login_card"], fg=COLORS["login_muted"],
                         font=("Segoe UI", 9, "underline"), cursor="hand2")
...
class DashboardPage(tk.Frame):
    def __init__(self, parent, app: App):
        ...
        self.balance_canvas.create_oval(45, 245, 245, 305, fill="#0369a1", outline="#0369a1")
        self.balance_canvas.create_oval(60, 255, 230, 300, fill="#0284c7", outline="#0284c7")
        self.balance_canvas.create_oval(75, 262, 215, 295, fill="#0ea5e9", outline="#0ea5e9")

        self.balance_canvas.create_oval(18, 18, 272, 272, fill="#082f49", outline="#082f49")
        self.balance_canvas.create_oval(34, 34, 256, 256, fill="#0c4a6e", outline="#0c4a6e")
        self.balance_canvas.create_oval(50, 50, 240, 240, fill="#0369a1", outline="#0369a1")
        ...
        self.center_value_id = self.balance_canvas.create_text(
            145, 158, text="", fill="#38bdf8", font=("Segoe UI", 14, "bold")
        )
        ...
        section.pack(fill="both", expand=True, padx=18, pady=(12, 10))

        # Centered button frame
        btn_container = tk.Frame(section, bg=COLORS["bg"])
        btn_container.pack(expand=True)

        row = tk.Frame(btn_container, bg=COLORS["bg"])
        row.pack()
